...
export class EnhancementComponent implements OnInit {
    ...
    private reduceSaltAndPepperNoise(median:boolean): void {
        if(median)this.enhancementService.applyMedianFilter(this.resultImageMatrix);
        else this.enhancementService.applyAverageFilter(this.resultImageMatrix);
        this.displayMatrix();
    }
    applyAverageFilter():void{
        this.reduceSaltAndPepperNoise(false);
    }
    applyMedianFilter():void{
        this.reduceSaltAndPepperNoise(true);
    }
